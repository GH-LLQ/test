...
(function(window){
    ...
    var Game = {
        ...
        binding: function(){
            window.addEventListener("keydown", this.buttonDown); //监听事件
            window.addEventListener("keyup", this.buttonUp);
            window.addEventListener("keypress", this.keyPressed);
            this.c.addEventListener("click", this.clicked);
        },

        clicked: function(){ //暂停、开始
            if(!Game.paused) {
                Game.pause();
            } else {
                if(Game.isGameOver){
        ...
        buttonUp: function(e){
            ...
            if(e.keyCode === 37 || e.keyCode === 65){
                Game.player.movingLeft = false; //左移
            }
            if(e.keyCode === 39 || e.keyCode === 68){
                Game.player.movingRight = false; //右移
            }
        },
        ...
        invincibleMode: function(s){ //无敌模式
        ...
        clear: function(){ //清除
            this.ctx.fillStyle = Game.color;  //设置填充颜色
            this.ctx.fillRect(0, 0, this.c.width, this.c.height); //绘制以填充颜色的矩形
        },
        ...
        loop: function(){ //循环
            if(!Game.paused){
                Game.clear();
                for(var i in Game.enemies){ //敌机
                    var currentEnemy = Game.enemies[i];
                    currentEnemy.draw(); //默认情况下draw()函数会无限重复绘图
                    currentEnemy.update();
                    if(Game.currentFrame % currentEnemy.shootingSpeed === 0){
                        currentEnemy.shoot();
                    }
                }
                for(var x in Game.enemyBullets){ //敌机子弹
    ...
    Player.prototype.draw = function(){ //绘制
    ...
    Enemy.prototype.update = function(){ //更新敌机
        if(this.movingLeft){
            if(this.x > 0){
                this.x -= this.speed;
                this.y += this.vy;
        ...
        for(var i in Game.bullets){
            ...
            if(Game.collision(currentBullet, this)){ //碰撞 死亡
                this.die();
    ...
    Enemy.prototype.die = function(){
        ...
        if(Game.enemiesAlive < Game.maxEnemies){ //生成敌机
            Game.enemiesAlive++;
            setTimeout(function(){
                new Enemy();
            }, 2000);
    ...
    Enemy.prototype.explode = function(){ //敌机爆炸
    ...
    var EnemyBullet = function(x, y, color){ //初始化敌机子弹
        this.width = 8;
        this.height = 20;
        this.x = x;
        this.y = y;
    ...
    var Particle = function(x, y, color){ //初始化敌机
        this.x = x;
        this.y = y;
    ...
    Particle.prototype.draw = function(){
        ...
        this.life++;
        if(this.life >= this.maxlife){  //删除敌机
